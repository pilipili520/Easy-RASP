...
import com.wangziyu666.easyrasp.Transformer;
...
public class ProcessHook {//
    ...
    public static Object[] processHook(Object self, Object[] objects) throws Exception {
        if (objects == null) {
            throw new SecurityException(String.format(" RASP banned not allowed."));
        }
...
//下面那个是获取表单数据执行，对request.getParameterMap()的返回值使用泛型时应该是Map<String,String[]>形式
        ...
        for (String key : params.keySet()) {
            ...
            for (String value : values) {
                ...
                if (value.equals(cmd)) { // 执行请求中参数一样都指令, 用无害指令替换掉这个指令，操纵字节码
